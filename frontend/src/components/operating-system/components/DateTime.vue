...
<template>
  <a-popover trigger="click"
    ...
    <template #content>
      <div class="select-none bg-white p-4 rounded-lg flex gap-0 flex-nowrap min-w-64 shadow-lg">
        <div class="flex flex-col flex-1">
          <div class="flex items-center gap-0">
            <div class="text-3xl font-bold">{{ dayjs().format('D') }}</div>
            <div class="w-[1px] bg-slate-500 h-6 ml-2"></div>
            <div class="writing-vertical-lr select-none text-xs font-bold">{{ dayjs().format('ddd') }}</div>
          </div>
        </div>
