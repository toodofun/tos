...
const currentTime = ref<HolidayDetail>()
const next7DaysEvent = ref<Array<HolidayDetail>>([])

useTimer(async () => {
  currentTime.value = await getHolidayDetail()
  ...
  <div
    ...
    <div class="flex flex-col flex-1 gap-4">
      <div class="flex items-center gap-0">
        <div class="text-5xl font-bold">{{ currentTime?.date.format('D') }}</div>
        <div class="w-[1px] bg-slate-500 h-6 ml-2 mr-1"></div>
        <div class="writing-vertical-lr select-none font-bold">{{ currentTime?.date.format('ddd') }}</div>
      </div>
      <div class="h-full w-full flex items-start justify-start text-xs text-gray-500">
        <div class="text-xs font-bold text-nowrap flex gap-1 items-center" v-for="(tag, index) in currentTime?.tags"
             :key="index">
